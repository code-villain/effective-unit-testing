...
쪼개진 논리를 해결하는 방법은 필요한 외부 데이터와 코드를 모두 테스트 안으로 이주시키는 것이다.

데이터나 로직을 언제 통합해야 할까?
어떤 데이터와 로직은 통합하는게 좋고, 어떤건 분리된 채로 놔두는게 좋다.
무엇을 끌어안고 무엇을 밀쳐둬야 할지 파악하는 팁을 알아보자
- 짧다면 통합하라
- 통합하기에 너무 길다면 팩토리 메서드나 테스트 데이터 생성기를 통해 만들어라
- 이마저도 어려우면 그냥 독립파일로 냄겨라

가장 좋은 방법은? 필요한 모든 정보를 테스트 안에 두는것이다.

하지만 만약 분리할 수 밖에 없는 상황이라면? 아래 지침을 기억하자
- 확인하려는 시나리오에 꼭 필요한 최소한의 데이터만 남긴다. 우발적 복잡성을 최소화 하는것은 필수다
- 데이터 파일과 이를 사용하는 테스트를 같은 폴더에 둔다.

### 4.6.3 정리

쪼개진 논리 냄새는 테스트의 논리나 데이터가 필요 이상으로 여러 곳으로, 여러 파일로 분리되어 있다는 경고다.


## 4.7 매직 넘버

매직넘버란 소스코드 중 할당문이나 메서드 호출 등에 박혀 있는 숫자로 된 값을 말한다.

매직넘버가 나쁜 이유는 뜻을 알 수 없기 때문이다.
매직넘버를 처리하는 전통적인 방법은 의미가 분명한 이름의 상수나 변수로 대체해서 읽기 쉬운 코드로 만드는 것이다.

책에서 제시하는 방법은
정적 상수나 지역 변수로 안하고 그와 비슷한 이름을 메서드 이름으로 하고 파라미터로 값을 넘기는 방법이다.
메서드 방식이 유리해지려면 파라미터로 넘기는 수를 다양하게 적용하는 테스트를 여러개 만들어야 한다.

코드는 간결함 보다는 가독성을 우선해야 할 때가 있다. 의미 전달을 위해 부가적인 문장을 한 두줄 추가해야 할 때가 적지 않다.

TEN_PINS , TWELWE_TIMES 와 같은 정적 상수
pins() , times() 와 같은 메서드 방식

둘 중 어떤것을 사용할지는 ... 테스트 케이스 많이 안만들거면 상수, 만들거면 메서드!

## 4.8 셋업 설교

4.4 에서 테스트의 의도와 목적과 의미를 정확하게 전달하는 방법을 배웠다.
테스트 메서드에는 본질만을 담아 간결하게 유지하고 나머지 세부 내용은 도우미 메서드로 추출하는 방법을 많이 사용한다

### 4.8.1 예시

쪼개진 논리도 보이고 SRP 도 위반한다. 그리고 셋업 메서드가 너무~~ 비대하다.

픽스처는 테스트가 실행하는 어떤 것이라 할 수 있는데, 시스템 속성, 테스트 클래스에 정의된 상수,
셋업 메서드가 초기화한 private 멤버 등이 속한다.

셋업의 역할은 테스트를 실행하기 위한 상태와 필요한 객체를 미리 만들어 놓는 것이다.
...
- 알맞은 서술적 이름을 사용한다.
- 셋업 내의 추상화 수준을 통일한다.(켄트백 구현패턴 - 대칭성)
...
논리적 단계를 구분하고 서술적인 이름을 부여해서 더 명확해졌다.
그리고 일정한 수준으로 추상화되어 이해하기도 쉬워졌다.
...
- 기본 타입 단언은 고수준 개념을 너무 낮은 개념을 이용해서 검사한다. 추상화 수준을 일치시키자
- 광역단언은 사소한 내용때문에 테스트가 실패한다. 꼭 필요한 것만 테스트하자
- 모든 기능을 한번에 테스트 하려는 욕심을 버리자
- 셋업 메서드에는 필요한 픽스처만 선언하자.
